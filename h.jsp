...
.CSSTableGenerator td{
	vertical-align:middle;
		background:-o-linear-gradient(bottom, #a8547c 5%, #ffd4aa 100%);	background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #a8547c), color-stop(1, #ffd4aa) ); 
	background:-moz-linear-gradient( center top, #a8547c 5%, #ffd4aa 100% );
	filter:progid:DXImageTransform.Microsoft.gradient(startColorstr="#a8547c", endColorstr="#ffd4aa");	background: -o-linear-gradient(top,#a8547c,ffd4aa);
...
.CSSTableGenerator tr:first-child td{
		background:-o-linear-gradient(bottom, #ff56aa 5%, #007fff 100%);	background:-webkit-gradient( linear, left top, left bottom, color-stop(0.05, #ff56aa), color-stop(1, #007fff) );
	background:-moz-linear-gradient( center top, #ff56aa 5%, #007fff 100% );
	filter:progid:DXImageTransform.Microsoft.gradient(startColorstr="#ff56aa", endColorstr="#007fff");	background: -o-linear-gradient(top,#ff56aa,007fff);

	background-color:#ff56aa;
	border:0px solid #888989;
	text-align:center;
	...
	<div id="templatemo_wrapper">
      ...
      <p>&nbsp; </p>
        ...
        <div id="templatemo_content_wrapper">
			...
			<tr><td colspan="4" align="center" >View the Masterkey1 Requests</td></tr>
			<tr     bgcolor="#000000">
			<td width="172"  ><font  color="#CC0000"size="3"><strong>Name</strong></font></td>
			
            <td width="228"><font  color="#CC0000"size="3"><strong>Email</strong></font></td>
			
		<td width="228"><font  color="#CC0000"size="3"><strong>Requested Date</strong></font></td>
			
			<td width="228"><font  color="#CC0000"size="3"><strong>Key Provide</strong></font></td>
...
Statement st1=null;
String name=null,date=null,email=null,v1=null,msg1=null,fname=null ;

String n="waiting";
v1=(String)session.getAttribute("ll");

w=(Integer)session.getAttribute("id");

try
{
con=databasecon.getconnection();
 st=con.createStatement();   
 String sqll="select * from request1 where   status='"+n+"' ";
 rs=st.executeQuery(sqll);
while(rs.next()){
	
	fid=rs.getInt("id");
	session.setAttribute("jk",fid);
...
<a href="sendemail1.jsp?<%=fid%>">
<b>send key</b></a></strong></font></td>
