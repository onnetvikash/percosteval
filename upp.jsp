...
<%
		...
		try{	
			...
			PreparedStatement ps=conn.prepareStatement("select * from timer where uid = '"+user_id+"'");
		...
		try{	
			...
			Connection conn = databasecon.getconnection();
			PreparedStatement ps=conn.prepareStatement("select * from files where uid = '"+user_id+"'");
			ResultSet rst = ps.executeQuery();
			while(rst.next())
			{
...
query="update timer set end12='"+end12+"',end24='"+end24+"',sizeprize='"+sprz+"',usageprize='"+upz+"',totalprize='"+tpz+"',usagetime='"+fge+"',nof='"+nof+"',filesize='"+finalsizer+"',filename='"+filenaming+"',tfs='"+finalsizer1+"' where uid='"+user_id+"';";
	try{				
